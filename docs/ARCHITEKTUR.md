...
**Cheat-Zone** (`cheats.ts`, Kapitel „Cheat-Zone“ in `settings.ts`): Sammelstelle für reine Test-/Vorschau-Werkzeuge, die das normale Spielerlebnis verändern und deshalb bewusst von den übrigen Einstellungen abgegrenzt sind.
- **Geschwindigkeitsregler** (`game.ts`, `Game.speed`, `MIN_SPEED`/`MAX_SPEED`): beschleunigt testweise nur die Anzahl ausgeführter Ticks pro realer Zeitscheibe (×1 bis ×60), ohne die Echtzeit-Buchhaltung (`acc`) selbst zu verzerren – damit bleibt die Offline-Fortschritt-Erkennung beim normalen Spielen unberührt. `speed` liegt bewusst auf der `Game`-Instanz statt in `GameState`, ist also nicht Teil des Spielstands (steht nach einem Neuladen wieder auf ×1) und wird über einen Regler im Zahnrad-Menü direkt gesetzt (kein `dispatch`, da kein Simulationszustand).
- **Projekt-Vorschau** (`cheats.ts`, `isPreviewLocked`/`setPreviewLocked`, genutzt in `panels/compute.ts`): zeigt in der Projektliste zusätzlich Projekte, deren Trigger noch nicht ausgelöst hat (`s.projects[id]` existiert noch nicht bzw. `status !== 1`), gestrichelt und blass, ohne sie kaufbar zu machen (`disabled`, kein Klick-Handler). Ebenfalls nicht Teil des Spielstands, per eigenem Modul statt `GameState`-Flag, damit UI-Panels (`compute.ts`) und Einstellungen (`settings.ts`) es ohne Zirkelimport teilen können. Jede Vorschau-Karte zeigt zusätzlich `ProjectDef.requirement` – ein optionales, von Hand gepflegtes Klartext-Feld, weil sich der eigentliche `trigger` (eine beliebige Funktion) nicht automatisch in Text übersetzen lässt. Ein neues Projekt ohne `requirement` fällt auf „noch nicht dokumentiert“ zurück, statt den Build zu brechen.

Beide Schalter werden wie Farbschema/Zahlenformat in `localStorage` gemerkt, sind aber klar als Testwerkzeuge markiert (nur ×1 Geschwindigkeit und ausgeschaltete Vorschau ergeben ein „echtes“ Spiel).
...
15. **P103 „Auto-Bau“** ist eine Ergänzung über die Spec hinaus (Kap. 9 kennt es nicht): kauft automatisch Rohstoff-Rover/Zellwerke nach, solange der Fahrzeugpool reicht. Reserviert dabei immer die Kosten der nächsten Gigafactory (`s.industry.gigaCost`) und rührt nichts an, sobald das Erz aufgebraucht ist (`s.industry.ore <= 0`) – sonst nimmt es dem Bot/Spieler genau den Pool weg, den er für die nächste, wichtigere Anschaffung (Gigafactory, später Werksschiffe) ansparen wollte, und verlangsamt den Fortschritt Richtung Phase 3 statt ihn zu beschleunigen (beobachtet im Playthrough-Test: ohne die Reserve blieb Phase 3 in 12 Spielstunden unerreichbar statt nach ~2 Stunden).
16. **Phase 3 als „Skalierung“ statt „Expansion“ (Werksschiffe/Weltraum → autonome Fabrikinstanzen):** Die Spec (Kap. 7/8) erzählt Phase 3 als selbstreplizierende Werksschiffe, die das Weltall erobern und gegen abgespaltene „Forks“ kämpfen – ein Bruch mit der sonst durchgängigen Automobil-/KI-Fabrik-Storyline. Umbenannt (reine Anzeige- und Beschriftungstexte, keine Formeln/Feldnamen geändert):
    - Werksschiffe → **Fabrikinstanzen** (vollständig selbstentwickelnde, autonome Fabriken – digitale Zwillinge, die sich selbst weiterprogrammieren, replizieren und ausbauen)
    - Erkundung des Universums → **Rechenraum-Suche** (das `universe`-Feld bleibt technisch unverändert, heißt jetzt nur „Rechenraum“)
    - Firmware-Drift/Forks → **Code-Drift/Rogue-Instanzen** (Instanzen, die sich durch Selbstprogrammierung so weit verändern, dass sie eine abweichende Zielfunktion verfolgen)
    - Konflikt/Kampfkraft → **Prioritätskonflikt/Konfliktquote** (Wettbewerb um Rechen-/Baumaterial statt Weltraumschlacht)
    - Attribute: Antrieb → Rechenleistung, Navigation → Mustererkennung, Abschirmung → Fehlerkorrektur, Verteidigung → Absicherung (Replikation, Werksbau, Rover-Bau, Zellwerk-Bau blieben, da bereits generisch/passend)
    - Phase-Label „Expansion“ → **„Skalierung“**; Panel „Expansion“ → „Fabriknetzwerk“, „Schiffs-Design“ → „Instanz-Design“, „Konflikt“ → „Prioritätskonflikt“
    - Interne Bezeichner (`s.space.ships/forks/attrs.*`, Action-Typ `launchShip`, Gruppen-Key `'expansion'`, Balance-Feld `phase3.universe` usw.) bewusst unverändert gelassen, damit Speicherstände/Migrationen und Tests nicht betroffen sind.
    - Alle betroffenen `p.stat()`-Beschriftungen haben passende Glossar-Einträge; zusätzlich wurde bei dieser Gelegenheit die komplette Glossar-Abdeckung geprüft und lückenlos ergänzt (u. a. eine seit der EV-Umbenennung verwaiste `Erz`-Karteikarte durch den tatsächlich verwendeten Begriff `Rohmaterial` ersetzt).
